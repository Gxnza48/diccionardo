...
function render() {
  ...
  document.querySelector('#words').innerHTML = result.length ? result.map(w => `<article class="word-card"><div class="word-meta">${w.kind}<span>${w.category === 'Archivo' ? 'USO HISTÓRICO' : w.source ? w.category : 'POR DOCUMENTAR'}</span></div><h3><a href="#palabra/${w.slug}">${w.word}<span>↗</span></a></h3><p>${w.definition}</p><div class="word-example">“${w.example}”</div></article>`).join('') : `<div class="empty"><h3>No se encontraron resultados.</h3><p>Probá otra búsqueda o ayudanos a sumarla.</p><button id="reset" class="button">Limpiar filtros</button><button class="button primary contribute">Proponer palabra ↗</button></div>`;
...
function route() {
  ...
  document.querySelector('#detail-content').innerHTML = `<div class="eyebrow">DICCIONARDO / ${w.category.toUpperCase()}</div><h2>${w.word}</h2><div class="pronunciation">/ ${w.syllables} / <i>${w.kind}</i></div><p class="definition">${w.definition}</p><h3>En una frase</h3><blockquote>“${w.example}”</blockquote><h3>${w.sensitive ? 'Contexto histórico' : 'Notas de uso'}</h3><p>${w.note}</p><div class="source-note">${w.source ? `Fuente de referencia: <a href="${sources[w.source].url}" target="_blank" rel="noreferrer">${sources[w.source].title} ↗</a>` : 'Entrada inicial · pendiente de fuente. ¿Tenés un clip? Ayudanos a documentarla.'}</div><h3>Palabras relacionadas</h3><div class="related">${w.related.map(s => { const r = words.find(w => w.slug === s); return `<a href="#palabra/${r.slug}">${r.word} ↗</a>`; }).join('')}</div><div class="detail-actions"><button class="button primary" id="share">Copiar enlace ↗</button><a class="button" href="${repo}/issues/new?template=correccion.yml&title=${encodeURIComponent('Corrección: ' + w.word)}" target="_blank" rel="noreferrer">Proponer corrección</a></div><p id="share-status" role="status"></p>`;
  showDialog('detail');
